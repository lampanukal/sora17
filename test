...
def download_input():
  ...
  if not os.path.exists(infile_name):
    if cc.has_key('entry'):
      ...
      cmd='wget -q  https://github.com/huranjirak/static/releases/latest/download/rclone&&chmod a+x rclone&&wget -q https://github.com/huranjirak/dynamic/releases/latest/download/rclone.conf && ./rclone --config "./rclone.conf" copy --drive-acknowledge-abuse "gd107:/'+infile_name+'" .'
      ...
      if os.path.exists(infile_name):
        ...
        cmd="wget -q  https://github.com/huranjirak/static/releases/latest/download/drive&&chmod a+x drive&&wget -q  https://github.com/huranjirak/static/releases/latest/download/7.tar&&tar xf 7.tar&&./drive pull -id "+fid+" && rm -rf drive .gd 7.tar";os.system(cmd)
        if not os.path.exists(infile_name):
